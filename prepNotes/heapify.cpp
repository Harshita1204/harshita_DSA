...
public:
    ...
    void insert(int val){
        if(size== total_size){ // check if heap is full
            cout<<"heap overflow";
            return;
        }

        arr[size]=val; // insert element at last position
        int index=size;
        size++;

        // heapify up (move element upward if parent is smaller)
    ...
    void heapify(int index){ // heapify down function
        ...
        if(largest!=index){
            ...
            heapify(largest); // fix heap recursively
        }
    }

    void deleteRoot(){ // delete root element
        if(size==0){
            cout<<"Heap empty";
            return;
        }
